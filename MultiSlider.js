...
export default class MultiSlider extends React.Component {
  ...
  componentWillMount() {
    var customPanResponder = (start, move, end) => {
      return PanResponder.create({
        ...
        onPanResponderMove: (evt, gestureState) => move(gestureState, evt),
  ...
  componentWillReceiveProps(nextProps) {
    ...
    if (nextState != {}) {
      ...
      if (nextState.positionOne !== this.positionOne)
        this.props.updatePosition(nextState.positionOne)
    }
  }
    ...
    if (Math.abs(gestureState.dy) < slipDisplacement || !slipDisplacement) {
      ...
      this.setState(
        ...
        () => {
          ...
          this.props.onValuesChange(change, this.state.positionOne)
        },
      )
    }
  }
